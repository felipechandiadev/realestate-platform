...
import { Select, Option } from "@realestate/ui"'
  ...
  if (mobileMode) {
    return (
      <div className="flex items-center gap-2 text-xs text-foreground" data-test-id="data-grid-pagination">
        <IconButton icon="first_page" variant="text" className="p-1 text-secondary cursor-pointer" onClick={() => handlePageChange(1)} aria-label="Primera página" disabled={page === 1} />
        <IconButton icon="chevron_left" variant="text" className="p-1 text-secondary cursor-pointer" onClick={() => handlePageChange(Math.max(1, page - 1))} aria-label="Anterior" disabled={page === 1} />
        ...
        <IconButton icon="chevron_right" variant="text" className="p-1 text-secondary cursor-pointer" onClick={() => handlePageChange(Math.min(totalPages, page + 1))} aria-label="Siguiente" disabled={page === totalPages} />
        <IconButton icon="last_page" variant="text" className="p-1 text-secondary cursor-pointer" onClick={() => handlePageChange(totalPages)} aria-label="Última página" disabled={page === totalPages} />
      </div>
    );
  }
