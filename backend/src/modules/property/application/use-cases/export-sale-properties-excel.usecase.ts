...
import { GridSalePropertiesUseCase } from './grid-sale-properties.usecase';

@Injectable()
export class ExportSalePropertiesExcelUseCase {
  constructor(private readonly gridSalePropertiesUseCase: GridSalePropertiesUseCase) {}
  ...
  async execute(query: any): Promise<Buffer> {
    ...
    const gridResult = await this.gridSalePropertiesUseCase.execute({ ...query, fields, pagination: 'false' });
