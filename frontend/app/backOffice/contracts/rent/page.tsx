/**
 * Rent Contracts Management Page
 * 
 * Propósito:
 * - Gestionar contratos de ARRIENDO de propiedades
 * - Visualizar estado de contratos (activo, vencido, renovación)
 * - Búsqueda y filtrado por múltiples criterios
 * - Ordenamiento por fecha, monto de arriendo, inquilino, estado
 * - Acciones: crear, editar, renovar, finalizar contratos
 * 
 * Funcionalidad:
 * - Server component: recibe searchParams (sort, search, filters, page, limit)
 * - Fetcha datos de contratos de arriendo desde getRentContractsGrid action
 * - Renderiza RentContractsGrid con datos y paginación
 * - Soporta URL-based filtering (estado, vigencia, etc.)
 * 
 * Audiencia: Administradores, Gestores de contratos, Abogados
 */

import RentContractsGrid from './ui/RentContractsGrid';
import { getRentContractsGrid } from '@/features/backoffice/contracts/actions/contracts.action';
...
export default async function RentContractsPage({ searchParams }: PageProps) {
  ...
  try {
    const result = await getRentContractsGrid({
      sort,
      sortField,
      search,
      filters,
    ...
    return (
      <div className="p-4">
        ...
        <RentContractsGrid rows={result.data} totalRows={result.total} />
      </div>
    );
  } catch (error) {
