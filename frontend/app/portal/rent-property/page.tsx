/**
 * Rent Property Publication Page (Portal)
 * 
 * Propósito:
 * - Formulario multi-step para publicar propiedad en ARRIENDO
 * - Permitir a usuarios publicar propiedades para arrendar
 * - Wizard guiado con validación paso a paso
 * - Preview antes de publicar
 * 
 * Funcionalidad:
 * - Client component: StepperBaseForm con 4 pasos
 * - Paso 1: Tipo propiedad y operación (forzado a RENT)
 * - Paso 2: Características básicas (m2, habitaciones, baños, etc.)
 * - Paso 3: Ubicación (región, comuna, coordenadas con mapa)
 * - Paso 4: Precio mensual, descripción, imágenes (FileUploader)
 ...
 * 
 * Audiencia: Usuarios registrados que quieren arrendar su propiedad
 */
...
import { listPropertyTypesPublic, getPropertyTypeCharacteristicsPublic, PropertyTypeWithFeatures, publishPropertyPublic, publishProperty } from '@/features/backoffice/properties/actions/properties.action';
    ...
    <div className="bg-card rounded-xl overflow-hidden border border-border shadow-md">
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="text-2xl font-bold text-primary">
          ...
          {operationType === 'RENT' && <span className="text-sm font-normal text-muted-foreground">/mes</span>}
        </div>
...
export default function RentPropertyPage() {
  ...
  useEffect(() => {
    if (session?.user) {
      ...
      setValues(prev => ({
        ...
        // session.user.phone comes from our custom next-auth session
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        console.log('📥 [RentProperty] Loading initial data...');
        const [typesResult, regionsResult] = await Promise.all([
          listPropertyTypesPublic(),
          getRegiones(),
        ]);

        console.log('📦 [RentProperty] Types result:', typesResult);
        console.log('📦 [RentProperty] Regions result:', regionsResult);

        if (typesResult.success && typesResult.data) {
          const mappedTypes = typesResult.data.map(pt => ({ id: pt.id, label: pt.name }));
          console.log('✅ [RentProperty] Property types loaded:', mappedTypes);
          setPropertyTypes(mappedTypes);
        } else {
          console.warn('⚠️ [RentProperty] Types result not successful:', typesResult);
        }

        setRegions(regionsResult);
      } catch (error) {
        console.error('❌ [RentProperty] Error loading initial data:', error);
      }
    };
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.log('📤 [handleSubmit] Enviando solicitud de arriendo...');
      ...
      const payload = {
        ...
        operationType: 'RENT' as const, // Siempre RENT para arriendo
      ...
      console.log('✅ Solicitud de arriendo enviada:', result.data.id);
  ...
  const buildStep1Fields = (): BaseFormField[][] => {
      ...
      [
        {
          ...
          type: 'text',
          required: true,
          props: { placeholder: 'Ej: Departamento 2D 1B en Providencia' }
        }
      ],
      [
        {
    ...
    // Add price and currency in the same row - Precio de arriendo mensual
    rows.push([
      {
        name: 'price',
  ...
  const steps: StepperStep[] = [
    {
      ...
      description: 'Título, tipo de propiedad y precio de arriendo',
      fields: step1Fields,
    },
    {
            ...
            <div className="flex justify-center">
              <div className="w-full max-w-md">
                <PropertyPreviewCard
                  ...
                  currencyPrice={(values.currencyPrice || 'CLP') as 'CLP' | 'UF'}
          ...
          <div className="flex flex-col items-center justify-center min-h-[400px] space-y-6">
            <div className="text-center space-y-4">
              ...
              <p className="text-lg text-muted-foreground max-w-md">
                Tu solicitud de arriendo ha sido recibida y será revisada por nuestro equipo en las próximas horas.
