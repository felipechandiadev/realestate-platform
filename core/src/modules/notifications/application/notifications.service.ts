...
export class NotificationsService {
  ...
  async findOne(id: string): Promise<Notification> {
    const notification = await this.findOneInternal(id);
    return this.externalizeNotification(notification);
  }

  /** Carga la entidad en forma interna (enums de BD). No externalizar antes de save. */
  private async findOneInternal(id: string): Promise<Notification> {
  ...
  async markAsOpened(id: string, viewerId: string): Promise<Notification> {
    ...
    notification.firstViewerId = viewerId;
    notification.firstViewedAt = new Date();
