...
export class AuthController {
  ...
  @Post('sign-in')
  @ApiOperation({
    summary: 'Sign in (legacy) — audience derived from role',
    deprecated: true,
  })
  ...
  @Post('staff/sign-in')
  @ApiOperation({ summary: 'Staff sign-in (ADMIN / AGENT) for backoffice' })
  @ApiBody({ type: LoginDto })
  @Audit(AuditAction.LOGIN, AuditEntityType.USER, 'Staff login attempt')
  async staffSignIn(@Body(ValidationPipe) loginDto: LoginDto) {
    return this.authService.signInStaff(loginDto);
  }

  @Post('community/sign-in')
  @ApiOperation({ summary: 'Community sign-in for portal' })
  @ApiBody({ type: LoginDto })
  @Audit(AuditAction.LOGIN, AuditEntityType.USER, 'Community login attempt')
  async communitySignIn(@Body(ValidationPipe) loginDto: LoginDto) {
    return this.authService.signInCommunity(loginDto);
  }
  ...
  @Post('register')
  @ApiOperation({
    summary: 'Register new community user (legacy alias)',
    deprecated: true,
  })
  ...
  @ApiOperation({ summary: 'Register new community user (portal)' })
  @ApiBody({ type: CreateUserCommunityDto })
  @Audit(AuditAction.CREATE, AuditEntityType.USER, 'Community user registration')
  async communityRegister(
    @Body(ValidationPipe) createUserCommunityDto: CreateUserCommunityDto,
  ) {
    return this.authService.register(createUserCommunityDto);
  }
