# Real Estate Platform — Core API

API NestJS (ex-`backend/`). Fuera del workspace npm de las apps frontend.

Ver `doc/PLAN_PASO_A_PASO_SPLIT.md`.
...
En local el SMTP apunta a Mailpit (catcher; no entrega a internet).

Desde la **raíz del monorepo**:

```bash
npm run mail:dev          # usa Mailpit ya levantado, o docker compose (:1025 / :8025)
# reiniciar core para cargar MAIL_* del .env
```

`mail:dev` es idempotente: si Homebrew/kai ya tienen Mailpit en esos puertos, no vuelve a levantar el contenedor.

Variables típicas en `core/.env`:
...
MAIL_USER=
MAIL_PASS=
MAIL_FROM=noreply@estateflow.local
```
...
curl -X POST http://localhost:8000/mail/test \
  ...
  -d '{"email":"dev@test.local"}'
```

Abrir http://localhost:8025 para ver el mensaje.
