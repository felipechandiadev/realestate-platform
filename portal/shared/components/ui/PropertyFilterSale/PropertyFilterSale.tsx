...
import { NumberStepper } from "@realestate/ui";
import { IconButton } from "@realestate/ui";
import { FilterSalePropertiesDto } from '@/features/properties/actions/saleProperties.action';
...
export default function PropertyFilterSale({
  ...
  const updateURL = useCallback((newFilters: FilterSalePropertiesDto) => {
    ...
    const newUrl = `/properties/sale?${params.toString()}`;
  ...
  useEffect(() => {
    ...
    async function loadIdentity() {
      try {
        const id = await (await import('@/features/cms/actions/identity.action')).getIdentity();
