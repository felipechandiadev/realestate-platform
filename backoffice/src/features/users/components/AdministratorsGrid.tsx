...
/**
 * Administrators Grid Component
 *
 * Displays a paginated, sortable grid of admin users
 * Supports searching, filtering, edit and delete actions
 *
 * @param {AdministratorsGridProps} props - Component props
 * @returns {React.ReactNode} Grid component
 */
export function AdministratorsGrid({ onEdit, onRefresh }: AdministratorsGridProps) {
  ...
  // Fetch users (filtered by admin roles)
  const {
    data: usersResponse,
    isLoading,
    error,
  ...
  // Map to grid format
  const gridItems: AdminGridItem[] = useMemo(
    () =>
      users.map((user) => ({
        id: user.id,
        email: user.email,
  ...
  // Handle delete click
  const handleDeleteClick = useCallback((item: AdminGridItem) => {
